...
if command -v apt >/dev/null 2>&1; then
    alias syu="time { sudo apt update && apt list --upgradable && sudo apt upgrade -y && sudo apt autoclean -y && sudo apt autopurge -y && sudo apt autoremove -y; }"
...
alias compile-qxy="sudo echo && time { sudo make uninstall -j && make clean && make test -j && sudo make install -j && ls -la /usr/local/bin/qxy* ; }"
